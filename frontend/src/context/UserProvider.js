...
const UserProvider = ({children})=> {
    ...
    const signUp = async (user) => {
        try {
            const response = await axios.post('https://bottlo.onrender.com/api/users', user)
    ...
    const login = async (user) => {
        try{
            const response = await axios.post('https://bottlo.onrender.com/api/users/login', user)
            if(response.data.success){
                dispatch({type : 'LOGIN', payload: response.data.token})
                Swal.fire({
                    position: 'center',
    ...
    const validateToken = async (token) => {
            const response = await axios.get('https://bottlo.onrender.com/api/users/login', {
                headers: {
    ...
    const getUserData = async (token) => {
        try{
            const response = await axios.get('https://bottlo.onrender.com/api/users', {
                headers: {
                    Authorization: 'Bearer ' + token
                }
            })
    ...
    const editUserData = async (user) => {
        try{const response = await axios.put('https://bottlo.onrender.com/api/users', user, {
            headers: {
